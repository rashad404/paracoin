                ...
                <div class="inline-block min-w-full py-2 align-middle">
                    <div class="overflow-hidden ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        ...
                        <thead class="bg-gray-50">
                            ...
                            <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">@lang('text.Address')</th>
                        ...
                        <tbody class="bg-white">
                            @foreach ($list as $item)
                                <tr class="hover:bg-gray-100 {{($loop->iteration % 2 == 0) ? 'bg-gray-50' : ''}}">
                                    ...
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{$address==$item->to ? $item->from: $item->to}}</td>
                                    @if ($address==$item->to)
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500"><span style="color:green">+{{$item->amount}}</span> {{config('coin.symbol')}}</td>
                                    @else
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500"><span style="color:red">-{{$item->amount}}</span> {{config('coin.symbol')}}</td>
                                    @endif
                                </tr>
                            @endforeach
                        </tbody>
