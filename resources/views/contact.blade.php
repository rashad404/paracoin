  ...
  <div class="bg-gray-100">
    <div class="max-w-7xl mx-auto py-2 px-4 sm:py-24 sm:px-6 lg:px-8">
      <div class="relative bg-white shadow-xl">
        <h2 class="sr-only">@lang('text.'.$pageTitle)</h2>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3">
          ...
          <div class="relative overflow-hidden py-10 px-6 bg-indigo-700 sm:px-10 xl:p-12">
            ...
            <h3 class="text-lg font-medium text-white">@lang('text.Contact Information')</h3>
            <p class="mt-6 text-base text-indigo-50 max-w-3xl"> @lang('text.We\'d love to hear from you! Send us a message using the form opposite, or email us.')</p>
            <dl class="mt-8 space-y-6">
              <dt><span class="sr-only">@lang('text.Phone number')</span></dt>
              <dd class="flex text-base text-indigo-50">
                ...
                <span class="ml-3">+1 (213) 444-3306</span>
              </dd>
              <dt><span class="sr-only">@lang('text.Email')</span></dt>
              <dd class="flex text-base text-indigo-50">
                <!-- Heroicon name: outline/mail -->
                <svg class="flex-shrink-0 w-6 h-6 text-indigo-200" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
                <span class="ml-3">{{config('coin.email')}}</span>
              </dd>
            </dl>
          ...
          <div class="py-10 px-6 sm:px-10 lg:col-span-2 xl:p-12">
            <h3 class="text-lg font-medium text-gray-900">@lang('text.Send us a message')</h3>
            <form action="/contact" method="POST" class="mt-6 grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-8">
              @csrf
              <div class="sm:col-span-2">
                <label for="name" class="block text-sm font-medium text-gray-900">@lang('text.Your name')</label>
                <div class="mt-1">
                  <input type="text" value="{{old('name', $name)}}" name="name" id="name" autocomplete="given-name" class="py-3 px-4 block w-full shadow-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                </div>
              </div>
              <div>
                <label for="email" class="block text-sm font-medium text-gray-900">@lang('text.Email')</label>
                <div class="mt-1">
                  <input id="email" value="{{old('email', $email)}}" name="email" type="email" autocomplete="email" class="py-3 px-4 block w-full shadow-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                </div>
              </div>
              <div>
                <label for="phone" class="block text-sm font-medium text-gray-900">@lang('text.Phone')</label>
                <div class="mt-1">
                  <input type="text" value="{{old('phone', $phone)}}" name="phone" id="phone" autocomplete="tel" class="py-3 px-4 block w-full shadow-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md" aria-describedby="phone-optional">
                </div>
              </div>
              <div class="sm:col-span-2">
                <label for="subject" class="block text-sm font-medium text-gray-900">@lang('text.Subject')</label>
                <div class="mt-1">
                  <input type="text" value="{{old('subject', $subject)}}" name="subject" id="subject" class="py-3 px-4 block w-full shadow-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                </div>
              </div>
              <div class="sm:col-span-2">
                <div class="flex justify-between">
                  <label for="message" class="block text-sm font-medium text-gray-900">@lang('text.Message')</label>
                  <span id="message-max" class="text-sm text-gray-500">@lang('text.Max. 500 characters')</span>
                </div>
                <div class="mt-1">
                  <textarea id="message" name="message" rows="4" class="py-3 px-4 block w-full shadow-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500 border border-gray-300 rounded-md" aria-describedby="message-max">{{old('message', $message)}}</textarea>
                </div>
              </div>
              <div class="sm:col-span-2 sm:flex sm:justify-end">
                <button type="submit" class="mt-2 w-full inline-flex items-center justify-center px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:w-auto">@lang('text.Submit')</button>
              </div>
            </form>
          </div>
        </div>
